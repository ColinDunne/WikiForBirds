...
-(void)applicationDidFinishLaunching:(UIApplication *)application {
    ...
    itemVO.initialName = [[NSString stringWithFormat:@"%c",pinyinFirstLetter([itemVO.chineseName characterAtIndex:0])] uppercaseString];
    ...
    [self birdsOrderByInitialName:birds];
    
    rootController.birds = birds;
}

- (void)birdsOrderByInitialName:(NSMutableArray *)birds {
    [birds sortUsingComparator:^NSComparisonResult(id obj1, id obj2) {
        NSString *name1 = [(WFBBirdVO *)obj1 initialName];
        NSString *name2 = [(WFBBirdVO *)obj2 initialName];
        NSComparisonResult result = [name1 compare:name2];
        
        return result == NSOrderedDescending; // 升序排列
    }];
}
